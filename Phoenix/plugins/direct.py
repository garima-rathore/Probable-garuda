...
@bot.on(phoenix_cmd(outgoing=True, pattern=r"direct(?: |$)([\s\S]*)"))
@bot.on(sudo_cmd(allow_sudo=True, pattern=r"direct(?: |$)([\s\S]*)"))
async def direct_link_generator(request):
    if request.fwd_from:
        return
    phoenixevent = await edit_or_reply(request, "`Processing...`")
    ...
    if message:
        ...
        await phoenixevent.edit("`Usage: .direct <url>`")
        return
    reply = ""
    links = re.findall(r"\bhttps?://.*\.\S+", message)
    if not links:
        reply = "`No links found!`"
        await phoenixevent.edit(reply)
    for link in links:
