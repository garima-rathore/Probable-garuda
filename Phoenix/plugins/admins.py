...
async def promote(promt):
    ...
    try:
        ...
        await hellevent.edit(f"**🔥 Pironoted   [{user.first_name}](tg://user?id={user.id})  Successfully In**  `{promt.chat.title}`!! \n**Admin Tag :**  `{rank}`")
...
async def muth(hell):
    if hell.is_private:
        ...
        if hell.chat_id == me_izz_shreef:
            return await eod(hell, "Nashe me hai kya chicha 🥴")
        try:
            mute(hell.chat_id, hell.chat_id)
        except Exception as e:
            await eor(hell, f"**Error **\n`{str(e)}`")
        else:
            await eor(hell, "**Chup Reh unkil🥴\n`**｀-´)⊃━☆ﾟ.*･｡ﾟ **`")
    else:
        ...
        if userid == me_izz_shreef:
            return await eod(hellevent, "Nashe me hai kya chicha", 5)
