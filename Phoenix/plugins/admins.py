...
from . import *
from phoenix.sql.mute_sql import is_muted, mute, unmute
...
NO_ADMIN = "😪 I am not an admin here sur !! "
...
@bot.on(phoenix_cmd(pattern="setgpic$"))
...
@bot.on(phoenix_cmd(pattern="promote(?: |$)(.*)"))
...
async def promote(promt):
    ...
    phoenixevent = await eor(promt, "`Promoting User...`")
    ...
    try:
        ...
        await phoenixevent.edit(f"**🔥 Pironoted sur  [{user.first_name}](tg://user?id={user.id})  Successfully In**  `{promt.chat.title}`!! \n**Admin Tag :**  `{rank}`")
...
@bot.on(phoenix_cmd(pattern="demote(?: |$)(.*)"))
...
async def demote(dmod):
    ...
    if not admin and not creator:
        ...
        return
    phoenixevent = await eor(dmod, "`Demoting User...`")
    rank = "ǟɖʍɨռ"
    ...
    try:
        ...
        await phoenixevent.edit(NO_PERM)
        return
    await phoenixevent.edit(f"**😪 Demoted  [{user.first_name}](tg://user?id={user.id})  Successfully In**  `{dmod.chat.title}`")
...
@bot.on(phoenix_cmd(pattern=r"mute ?(.*)"))
@bot.on(sudo_cmd(pattern=r"mute ?(.*)", allow_sudo=True))
async def muth(phoenix):
    if phoenix.is_private:
        await eor(phoenix, "**Enough of your bullshit  !!**")
        await sleep(2)
        await phoenix.get_reply_message()
        replied_user = await phoenix.client(GetFullUserRequest(hell.chat_id))
        if is_muted(phoenix.chat_id, phoenix.chat_id):
            return await phoenix.edit(
                "Nigga is already muted here 🥴"
            )
        if phoenix.chat_id == me_izz_shreef:
            return await eod(phoenix, "Nashe me hai kya chicha 🥴 Bot developer ko mute krega?????")
        try:
            mute(phoenix.chat_id, phoenix.chat_id)
        except Exception as e:
            await eor(phoenix, f"**Error **\n`{str(e)}`")
        else:
            await eor(phoenix, "**Chup Reh unkil🥴\n`**｀-´)⊃━☆ﾟ.*･｡ﾟ **`")
    else:
        phoenixevent = await eor(phoenix, "`Muting...`")
        input_str = phoenix.pattern_match.group(1)
        chat = await phoenix.get_chat()
        if phoenix.reply_to_msg_id:
            userid = (await phoenix.get_reply_message()).sender_id
            name = (await phoenix.client.get_entity(userid)).first_name
        elif input_str:
            if input_str.isdigit():
                try:
                    ...
                    return await phoenixevent.edit(str(ve))
            else:
                userid = (await phoenix.client.get_entity(input_str)).id
                name = (await phoenix.client.get_entity(userid)).first_name
        else:
            return await eod(phoenixevent, "I Need a user to mute!!", 5)
        if userid == me_izz_shreef:
            return await eod(phoenixevent, "Nashe me hai kya chicha", 5)
        if str(userid) in DEVLIST:
            return await eod(phoenixevent, "**Error Muting God**", 7)
        try:
            await phoenix.client.edit_permissions(
                chat.id,
                userid,
                until_date=None,
                send_messages=False,
            ...
            await eor(phoenixevent, f"`{str(be)}`")
        await phoenix.client.send_message(
            lg_id,
            "#MUTE\n"
...
@bot.on(phoenix_cmd(pattern=r"unmute ?(.*)"))
...
async def nomuth(evn):
    if evn.is_private:
        ...
        phoenixevent = await eor(evn, "`Unmuting...`")
        ...
        if evn.reply_to_msg_id:
            ...
            return await eod(phoenixevent, "Oo vaii kisko mute kru!!", 3)
        try:
            ...
            await eor(
                phoenixevent,
                f"**Successfully chup kara diya vaii**  [{name}](tg://user?id={userid}) **in**  `{chat.title}`",
            )
        except BaseException as be:
            await eor(phoenixevent, f"`{str(be)}`")
...
@bot.on(phoenix_cmd(pattern="ban(?: |$)(.*)"))
@bot.on(sudo_cmd(pattern="ban(?: |$)(.*)", allow_sudo=True))
@errors_handler
async def ban(bon):
    if bon.fwd_from:
        return
    phoenixevent = await eor(bon, "`Banning Nigga...`")
    ...
    if not user:
        return await phoenixevent.edit("`Reply to a user or give username!!`")
    if str(user.id) in DEVLIST:
        return await phoenixevent.edit("**Say again? Ban my creator??**")
    try:
    ...
    try:
        ...
        await phoenixevent.edit(f"**Banned  [{user.first_name}](tg://user?id={user.id})  in** `[{bon.chat.title}]` !!\n\nMessage Nuking : **False**")
        return
    if reason:
        await phoenixevent.edit(f"**Bitch** [{user.first_name}](tg://user?id={user.id}) **is now banned in**  `[{bon.chat.title}]` !!\n**Reason :** `{reason}`")
    else:
        await phoenixevent.edit(f"**Bitch** [{user.first_name}](tg://user?id={user.id}) **is now banned in**  `[{bon.chat.title}]`!!")
...
@bot.on(phoenix_cmd(pattern="unban(?: |$)(.*)"))
...
async def nothanos(unbon):
    ...
    if not admin and not creator:
        ...
        return
    phoenixevent = await eor(unbon, "`Unbanning...`")
    ...
    try:
        ...
        await phoenixevent.edit(f"[{user.first_name}](tg://user?id={user.id}) **Is Now Unbanned in**  `{unbon.chat.title}` !!")
        ...
        await phoenixevent.edit("Invalid UserId!! Please Dobara check it!!")


@bot.on(phoenix_cmd(pattern="pin($| (.*))"))
...
@bot.on(phoenix_cmd(pattern="kick(?: |$)(.*)"))
...
async def kick(usr):
    ...
    phoenixevent = await eor(usr, "`Kicking...`")
    try:
        await usr.client.kick_participant(usr.chat_id, user.id)
        await sleep(0.5)
    except Exception as e:
        await phoenixevent.edit(NO_PERM + f"\n`{str(e)}`")
        return
    if reason:
        await phoenixevent.edit(
            f"**🏃 Kicked**  [{user.first_name}](tg://user?id={user.id})'s **Butt from** `{usr.chat.title}!`\nReason: `{reason}`"
        )
    else:
        await phoenixevent.edit(f"**🏃 Kicked**  [{user.first_name}](tg://user?id={user.id})'s **Butt from** `{usr.chat.title}!`")
...
@bot.on(phoenix_cmd(pattern=f"zombies ?(.*)"))
...
@bot.on(phoenix_cmd(pattern="undlt$"))
