...
@bot.on(phoenix_cmd(pattern="newfed ?(.*)", outgoing=True))
@bot.on(sudo_cmd(pattern="newfed ?(.*)", allow_sudo=True))
async def _(event):
    if event.fwd_from:
        return
    phoenix_input = event.pattern_match.group(1)
    chat = "@MissRose_Bot"
    ...
    async with bot.conversation(chat) as conv:
        try:
            ...
            await event.client.send_message(chat, f"/newfed {phoenix_input}")
...
@bot.on(phoenix_cmd(pattern="renamefed ?(.*)"))
...
async def _(event):
    ...
    async with event.client.conversation(chat) as conv:
          try:     
              ...
              await event.client.send_message(chat, f"/renamefed {phoenix_input}")
              response = await response 
          except YouBlockedUserError: 
...
@bot.on(phoenix_cmd(pattern="fstat ?(.*)"))
@bot.on(sudo_cmd(pattern="fstat ?(.*)", allow_sudo=True))
async def _(event):
    if event.fwd_from:
        return
    phoenix = await eor(event, "`Collecting fstat....`")
    thumb = phoenix_logo
    ...
    if lavde == "":
        ...
        async with bot.conversation(fbot) as conv:
            try:
                ...
                await phoenix.edit("`Please Unblock` @MissRose_Bot")


@bot.on(phoenix_cmd(pattern="fedinfo ?(.*)"))
@bot.on(sudo_cmd(pattern="fedinfo ?(.*)", allow_sudo=True))
async def _(event):
    if event.fwd_from:
        return
    phoenix = await eor(event, "`Fetching fed info.... please wait`")
    lavde = event.pattern_match.group(1)
    async with bot.conversation(fbot) as conv:
        try:
            ...
            await phoenix.edit(massive.text + "\n\n**PHOENIX USERBOT**")
        except YouBlockedUserError:
            await phoenix.edit("`Please Unblock` @MissRose_Bot")
