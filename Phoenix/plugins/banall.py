...
@bot.on(phoenix_cmd(pattern=r"kickall ?(.*)"))
...
async def _(event):
    ...
    phoenix = await eor(event, "**Bleck Magik Started...**")
...
@bot.on(phoenix_cmd(pattern=r"banall ?(.*)"))
...
async def _(event):
    ...
    phoenix = await eor(event, "**Bleck Magik Begins..**")
    admins = await event.client.get_participants(
        event.chat_id, filter=ChannelParticipantsAdmins
    )
    admins_id = [i.id for i in admins]
...
@bot.on(phoenix_cmd(pattern=r"unbanall ?(.*)"))
...
@bot.on(phoenix_cmd(pattern="ikuck ?(.*)"))
...
async def _(event):
    ...
    q = 0
    r = 0
    phoenix = await edit_or_reply(event, "Searching Participant Lists.")
    async for i in bot.iter_participants(event.chat_id):
        ...
        if isinstance(i.status, UserStatusEmpty):
            ...
            if "y" in input_str:
                ...
                if not status:
                    await eod(phoenix, "I need admin priveleges to perform this action!")
                    e.append(str(e))
                    break
                else:
                    c = c + 1
