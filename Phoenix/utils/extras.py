...
async def edit_or_reply(
    ...
    if aslink or deflink:
        linktext = linktext or "Message was too big so pasted it to bin"
        try:
            key = (
                requests.post(
...
# delete timeout
async def delete_phoenix(event, text, time=None, parse_mode=None, link_preview=None):
    parse_mode = parse_mode or "md"
    link_preview = link_preview or False
    time = time or 10
    if event.sender_id in Config.SUDO_USERS:
        ...
        phoenixevent = await event.edit(
            text, link_preview=link_preview, parse_mode=parse_mode
        )
    await asyncio.sleep(time)
    return await phoenixevent.delete()
