...
from phoenix.sql import sudo_sql as s_ql

#  normal user cmd
def phoenix_cmd(pattern=None, command=None, **args):
    ...
    if pattern is not None:
        if pattern.startswith(r"\#"):
            ...
            if len(Config.HANDLER) == 2:
                ...
                hellreg = "^\\" + Config.HANDLER
                reg = Config.HANDLER
            args["pattern"] = re.compile(phoenixreg + pattern)
    ...
    # blacklisted chats. 
    # Phoenix will not respond in these chats.
...
def sudo_cmd(pattern=None, command=None, **args):
    ...
    if pattern is not None:
        if pattern.startswith(r"\#"):
            ...
            if len(Config.SUDO_HANDLER) == 2:
                phoenixreg = "^" + Config.SUDO_HANDLER
    ...
    args["outgoing"] = True
    # outgoing check
    # Phoenix Userbot      @me_izz_shreef OP
    if allow_sudo:
        args["from_users"] = list(Config.SUDO_USERS)
        # Mutually exclusive with outgoing (can only set one of either).
        args["incoming"] = True
    ...
    # error handling condition check
        ...
        args["outgoing"] = True
    # blacklisted chats
    # Phoenix bot won't respond here
    args["blacklist_chats"] = True
    black_list_chats = list(Config.BL_CHAT)
    if black_list_chats:
        args["chats"] = black_list_chats
    ...
    if "allow_edited_updates" in args and args["allow_edited_updates"]:
        del args["allow_edited_updates"]
    # outgoing check
    # PHOENIX USERBOT MADE BY @me_izz_shreef
    return events.NewMessage(**args)
