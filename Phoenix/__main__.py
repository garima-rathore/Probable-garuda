...
hl = Config.HANDLER
HELL_PIC = Config.ALIVE_PIC or "https://telegra.ph/file/04cdefe5ed7732a67eaee.jpg"
...
if len(sys.argv) not in (1, 3, 4):
    ...
    try:
        if Config.BOT_USERNAME is not None:
            ...
            LOGS.info("🔰 Starting ☠ρøıƨøп ʙᴏᴛ☠ 🔰")
            bot.loop.run_until_complete(hell_bot(Config.BOT_USERNAME))
            LOGS.info("🔥 ☠ρøıƨøп ʙᴏᴛ☠ Startup Completed 🔥")
        else:
            bot.start()
    except Exception as e:
...
tbot.start()
LOGS.info("⚡ Your ☠ρøıƨøп ʙᴏᴛ☠ Is Now Working ⚡")
LOGS.info(
    "Head to @Its_HellBot for Updates. Also join chat group to get help regarding to ☠ρøıƨøп ʙᴏᴛ☠."
)
    ...
    try:
        if Config.LOGGER_ID != 0:
            await bot.send_file(
                Config.LOGGER_ID,
                HELL_PIC,
                caption=f"#START \n\nDeployed ☠ρøıƨøп ʙᴏᴛ☠ Successfully\n\n**☠ρøıƨøп ʙᴏᴛ☠ - {hellver}**\n\nType `{hl}ping` or `{hl}alive` to check! \n\nJoin [☠ρøıƨøп ʙᴏᴛ☠† Channel](t.me/Its_HellBot) for Updates & [☠ρøıƨøп ʙᴏᴛ☠ Chat](t.me/hellbot_chat) for any query regarding Hêllẞø†",
            )
    except Exception as e:
        LOGS.info(str(e))
